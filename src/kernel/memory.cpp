...
private:
	// Declaration order is load-bearing. Members are destroyed in reverse, so m_lock releases
	// the mutex before m_critical leaves the critical section -- and leaving it is what
	// dispatches any signal deferred while it was held. The other order would run the guest
	// handler with the memory mutex still held, which is the deadlock this guard exists to stop.
